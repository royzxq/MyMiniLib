...
public:
    ...
    T data;
#ifdef SMART_PTR
    std::shared_ptr<SegTreeNode> left, right;
#else
...
private:
#ifdef SMART_PTR
    std::shared_ptr<SegTreeNode<T> > root;
#else
    SegTreeNode<T> * root;
#endif
    ...
    comp _cmp;
#ifdef SMART_PTR
    std::shared_ptr<SegTreeNode<T>> _build(int start, int end);
    
    std::shared_ptr<SegTreeNode<T>> _build(std::vector<T> & vec, int start, int end);
    
    T _query(std::shared_ptr<SegTreeNode<T>> root, int start, int end);
    
    void _update(std::shared_ptr<SegTreeNode<T>> root, int index, T value);
#else
...
#ifdef SMART_PTR
template <class T>
std::shared_ptr<SegTreeNode<T>> SegTree<T>::_build(int start, int end) {
    if (end < start) {
        return nullptr;
    }
    std::shared_ptr<SegTreeNode<T>> node(new SegTreeNode<T>(start, end));
    if (start != end) {
        int mid = (start+end)/2;
...
template <class T>
std::shared_ptr<SegTreeNode<T>> SegTree<T>::_build(std::vector<T> & vec, int start, int end) {
    if (end < start) {
        return nullptr;
    }
    std::shared_ptr<SegTreeNode<T>> node(new SegTreeNode<T>(start, end));
...
template <class T>
T SegTree<T>::_query(std::shared_ptr<SegTreeNode<T> > root, int start, int end) {
...
template <class T>
void SegTree<T>::_update(std::shared_ptr<SegTreeNode<T> > root, int index, T value){
...
#ifdef SMART_PTR
    std::queue<std::shared_ptr<SegTreeNode<T>>> q;
#else 
